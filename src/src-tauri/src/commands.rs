...
#[tauri::command]
pub fn log_get_config() -> IpcResponse<LogConfig> {
    IpcResponse::ok(LogConfig {
        max_bytes: 1_048_576,
        max_files: 5,
    })
}
...
#[tauri::command]
pub fn backup_get_status(store: State<Store>) -> IpcResponse<BackupStatus> {
    let (latest_backup_at, count) = store.backup_status();
    IpcResponse::ok(BackupStatus {
        latest_backup_at,
        count,
    })
}
